...
| **C2** | Nav bar **"Sign In" / "Get Started" / "Pricing"**, and a signed-in **"Sachin"** avatar | ADR-0007: no accounts in MVP. The mockup contradicts *itself* — the hero also advertises "No Account Required" | **Cut Sign In, Get Started and Pricing from v1 nav.** Keep the "No Account Required" badge, which is the honest and stronger message. Nav in v1: Home · Features · How it Works. |
...
- Before a user's **first ever** share: a one-time notice — *"Only share with
